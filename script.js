var pins_Mode = [1,1,1,1,0,1,9,9,9,9,9,9,0,1,0,1,1]; // 1 for INPUT , 0 for OUTPUT
		...
		function drawChip(){
			...
			ctx.fillRect(0,0,konstW,konstH);
			ctx.fillStyle = "#999999";
			ctx.fillRect(konstW/10, konstH/3.3, konstW-konstW/5, konstH*(1-1/3.3-0.03));
			...
			ctx.fillRect(konstW/5.75,konstH/13.3,konstW/17.7,konstH/4.44);
		...
		function toggleAutoRead(){
			...
			if(auto_read == 0){
				// enable auto read
				interval = setInterval(autoRead, 2000);
		...
		function handleServerResponse_readMultiPins(){
			if(xmlHttp.readyState==4 && xmlHttp.status==200){
				xmlResponse=xmlHttp.responseXML;
				xmldoc_response = xmlResponse.getElementsByTagName('response');
				...
				for(var i=0;i<10;i++){
					var message = xmldoc_response[0].childNodes[i].nodeValue;
					console.log(message);
				}
				//message = xmldoc_response.childNodes[0].nodeValue;
